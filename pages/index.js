    ...
    <div className={`fixed inset-0 flex overflow-hidden ${darkMode ? 'bg-[#0a0f1e] text-white' : 'bg-white text-slate-900'}`}>
      ...
      <div className={`flex-1 flex flex-col transition-colors relative ${darkMode ? 'bg-[#0a0f1e]' : 'bg-[#F8FAFC]'} ${!showChatWindow && !showSettingsTab ? 'hidden' : 'flex'}`}>
        {showSettings || showSettingsTab ? (
          ...
          <>
            <div className={`p-4 md:p-6 border-b flex items-center justify-between transition-all duration-300 z-30 relative ${darkMode ? 'bg-[#111827] border-white/5' : 'bg-white border-slate-100'}`}>
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className={`font-black text-base uppercase tracking-wider ${darkMode ? 'text-white' : 'text-[#00337C]'}`}>{partnerUser?.name || partnerName}</p>
                        <p className="text-xs opacity-60">{partnerEmail}</p>
                      </div>
                    </div>
                  );
                })()}
            ...
            <div className={`p-4 md:p-6 relative z-10 ${darkMode ? 'bg-[#0a0f1e]' : 'bg-[#F8FAFC]'}`}>
              {replyTo && (
              ...
              <div className={`max-w-4xl mx-auto rounded-[30px] shadow-lg flex items-center gap-2 p-2 px-4 relative transition-all duration-200 ${darkMode ? 'bg-[#111827] border border-white/5' : 'bg-white border border-slate-100'} ${isRecording ? 'ring-4 ring-[#00337C]/30' : ''}`}>
                ...
                <button onClick={() => chatFileInputRef.current.click()} className="p-3 text-slate-400 hover:text-blue-400"><Paperclip size={20} /></button>
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-3 text-slate-400 hover:text-blue-400"><Smile size={20} /></button>
                <button onClick={() => isRecording ? stopRecording() : startRecording()} className={`p-3 ${isRecording ? 'text-red-400' : 'text-slate-400'} hover:text-blue-400`} title={isRecording ? 'Stop recording' : 'Start recording'}>
                  <Mic size={20} />
                </button>
          ...
          <div className="flex-1 flex flex-col items-center justify-center select-none">
            <img src={darkMode ? ASSETS.logoNameWhite : ASSETS.logoNameNavy} alt="Cove Logo" className="w-[200px] mb-6 opacity-15 transition-all duration-500" />
